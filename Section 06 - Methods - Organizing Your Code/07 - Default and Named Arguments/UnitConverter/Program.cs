...
while (running)
{
    ...
    else if (choice.ToLower() == "1")
    {
        ...
        bool parsed = double.TryParse(input, out double fahrenheit);

        if (parsed)
        {
            double celsius = CalculateCelsius(fahrenheit);
          
            Console.WriteLine($"{FormatOutput(fahrenheit)} °F = {FormatOutput(celsius)} °C");
            Console.WriteLine();
            Console.WriteLine($"High precision: {FormatOutput(celsius, decimals: 4)} °C");
        }
        else
        {
...
string FormatOutput(double value, int decimals = 2)
{
